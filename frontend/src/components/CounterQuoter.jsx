...
export function CounterQuoter() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Hero Search Box */}
      <div className="bg-gradient-to-br from-slate-900 via-slate-900 to-amber-950/30 border border-amber-500/30 rounded-3xl p-6 sm:p-8 shadow-2xl">
        <div className="max-w-3xl mx-auto space-y-4 text-center sm:text-left">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            ...
            <div className="text-xs text-slate-400 flex items-center gap-1 justify-center sm:justify-end">
          ...
          <div className="relative">
            ...
            <input
              ...
              autoFocus
              className="w-full pl-13 pr-4 py-4 bg-slate-950/90 border-2 border-slate-700 hover:border-slate-600 focus:border-amber-500 rounded-2xl text-base sm:text-lg text-white placeholder-slate-500 shadow-inner focus:outline-none transition font-medium"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-4 top-1/2 -translate-y-1/2 text-xs px-2 py-1 rounded bg-slate-800 text-slate-400 hover:text-white"
              >
                Limpiar
              </button>
            )}
          ...
          {/* Quick-Pick Motorcycle Model Chips */}
          <div className="flex items-center gap-1.5 flex-wrap justify-center sm:justify-start pt-1">
            ...
            {POPULAR_MODELS.map(model => (
              <button
                ...
                className="text-xs px-3 py-1 rounded-full bg-slate-800/80 hover:bg-amber-500/20 text-slate-300 hover:text-amber-300 border border-slate-700 hover:border-amber-500/40 transition font-medium"
              >
                {model}
              </button>
            ))}
      ...
      <div className="flex items-center justify-between px-2">
        ...
        <span className="text-xs text-slate-500">
          Prioridad: 🟢 Lista Oficial &gt; 🔵 Ref. Mercado &gt; 🟡 Ajuste Dólar
        </span>
      </div>
            ...
            return (
              ...
              >
                <div>
                  ...
                  {/* Top: Brand, Category, Source Badge */}
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <span className="text-[11px] font-bold px-2 py-0.5 rounded bg-slate-800 text-slate-300 border border-slate-700">
                      {p.brand}
                    </span>
                    
                    <div className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border flex items-center gap-1 ${sourceBadge.bg}`}>
